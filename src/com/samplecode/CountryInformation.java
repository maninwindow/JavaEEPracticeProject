...
public class CountryInformation extends HttpServlet {
	...
	private Country getInfo(String countryCode) {
		...
		try {
			...
			sql = "Select * from ChatBox where code = ?";
			...
			while (rs.next()) {
				...
				country.setLifeExpectancy(rs.getString("lifeExpectancy") == null ? new Double(0)
						: Double.parseDouble(rs.getString("lifeExpectancy").trim()));
				country.setGnp(
						rs.getString("gnp") == null ? new Double(0) : Double.parseDouble(rs.getString("gnp").trim()));
			}
